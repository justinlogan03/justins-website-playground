...
export default async function handler(req: any, res: any) {
  ...
  const nbaTeamScoresResponse = await Promise.all(
    nbaTeamIds.map(async (nbaTeamId) => {
      ...
      try {
        ...
        // Parse regular season wins from recordSummary (e.g. "57-25")
        ...
        console.error(`Failed to fetch data for NBA team ${nbaTeamId}:`, err);
      }
